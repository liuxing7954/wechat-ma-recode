...
function doWxss(dir, cb, mainDir, nowDir) {
    ...
    function runVM(name, code) {
        ...
        try {
            ...
            // 如果代码不完整，尝试修复
            if (code.includes('setCssToHead(') && !code.includes('}')) {
                code += '}';
            }
            ...
            for (let name in wxAppCode) {
                ...
                if (name.endsWith(".wxss")) {
                    try {
                        ...
                        console.log(`Error executing wxss for ${name}:`, e.message);
                    }
                }
            }
        } catch (e) {
            console.log(`Error in runVM for ${name}:`, e.message);
    ...
    wu.scanDirByExt(dir, ".html", files => {
        ...
        wu.get(frameFile, code => {
            ...
            try {
                ...
                if (!pureData) {
                    console.log("Warning: Failed to get pure data, trying to fix the code...");
                    // Try to fix the code by adding missing parts
                    if (code.endsWith('_C')) {
                        code = code + '={};';
                        pureData = vm.run(code + "\n_C");
                    }
                ...
                console.log("Error running VM for CSS:", e.message);
                pureData = {};
            }
